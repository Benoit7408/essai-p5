...
/*------------------ fonction asynchrone pour la réception des données suite au fetch sur un seul produit(route differente de page script) et la résolution des promesses -------------------------*/
...
/*------------------ fonction qui va chercher l'id du produit necessaire pour la route-------------------------*/
function findEltUrl(elt) {
  let EltUrl = new URLSearchParams(window.location.search);
  return EltUrl.get(elt);
}
/*------------------ fonction async qui retourne les promesses résolue du fetch -------------------------*/
...
/*------------------ fonction qui va afficher/construire les éléments dans le DOM -------------------------*/
...
/*------------------ fonction pour gestion de mauvaise quantité dans input-------------------------*/
...
function lStorage(panier) {
  ...
  if (eleInLstorage !== null) {
    ...
    eleInLstorage.forEach(function boucle(elem__it) {
      if (
        elem__it.idProduit == panier.idProduit &&
        elem__it.couleurProduit == panier.couleurProduit
      ) {
        elem__it.quantiteProduit =
          parseInt(panier.quantiteProduit) + parseInt(elem__it.quantiteProduit);
        window.alert("Votre produit est ajouté au panier");
        console.log(typeof elem__it.quantiteProduit);
        la__condition__est = true;
        console.log(
          `Des element identique dans le panier : ` + la__condition__est
        );
      }
    });
